...
while(~isempty(Q))
    ...
    [xhat, nFactors, nPotentials1, dual, U, V] = HungarianBP(NofNodes, NofStates, CNode.Factors, nPotentials1, CNode.U, CNode.V, 1, BaBOptions.bpoptions.innerIter,GLB);
    UB = dual;
    AllUB = [AllUB(:); dual];
    GUB = min(GUB, max(AllUB));
    LB = CluComputeObj(xhat, NofStates, Factors, Potentials);
    if(LB >= GLB)
        GLB = LB;
        x = xhat;
    end
