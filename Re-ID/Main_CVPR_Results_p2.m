...
DATASET = {'RAiD', 'WARD'}; % Dataset name
d_siz=size(DATASET,2);
cmc=cell(1,d_siz);
mbst_cmc=cell(1,d_siz);

No_Ex=1:5;% No of experiments, similar to [Das et al., ECCV 2014]
...
for jd=1:d_siz
    for trialNo=No_Ex
        ...
        BDs = loadTable2(DATASET{jd}, trialNo); % Loading assignment matrix (as cost)
        % Exactly the same cost matrix as [Das et al., ECCV 2014]
    ...
    save([pwd,filesep,'Results',filesep,'All_ReID_p2_m',num2str(mbest),'.mat'],...
